...
import { Activity, BarChart3, Building2, CircleDollarSign, Gauge, LayoutDashboard, Settings, SlidersHorizontal, Zap } from "lucide-react";
...
const copy = {
  overview: ["Good morning", "Your utility portfolio at a glance."],
...
export default function PortalShell({ children, header, companyName }) {
  ...
  return <div className="portal-layout">
    <aside className="portal-sidebar">
      ...
      <div><div className="portal-nav-label">Workspace</div><nav className="portal-nav">{items.slice(0, 7).map(([id, label, Icon]) => <Link key={id} href={`/dashboard?section=${id}`} className={section === id ? "active" : ""}><Icon size={17}/>{label}</Link>)}</nav></div>
      <div><div className="portal-nav-label">Management</div><nav className="portal-nav">{items.slice(7).map(([id, label, Icon]) => <Link key={id} href={`/dashboard?section=${id}`} className={section === id ? "active" : ""}><Icon size={17}/>{label}</Link>)}<Link href="/admin/rates"><SlidersHorizontal size={17}/>Supplier rates</Link></nav></div>
