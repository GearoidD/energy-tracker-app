...
async function convertHeicIfNeeded(file) {
  ...
  return new File([convertedBlob], (file.name || "photo").replace(/\.\w+$/, ".jpg"), { type: "image/jpeg" });
}

// Resizes and re-compresses before upload - a raw phone photo can be several MB,
// which is slow to upload over mobile data. This keeps enough detail for small
// print (rates, MPRN numbers) while cutting upload time significantly.
async function compressImage(file, maxDimension = 2200, quality = 0.88) {
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      const res = await fetch("/api/extract-bill", {
        ...
        body: JSON.stringify({ base64, mediaType: compressed.type }),
      });
